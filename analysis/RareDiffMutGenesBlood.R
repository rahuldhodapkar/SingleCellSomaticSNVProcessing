...
vdf_ms <- dbGetQuery(con, '
  SELECT * from rare_variants_by_gene
  WHERE 
      "Group" = \'MS\'
      AND "TissueType"=\'B\';
');
...
write.csv(df_cell, './ad_hoc/rare_diff_cell_blood.csv', row.names = F);
...
write.csv(df_donor, './ad_hoc/rare_diff_donor_blood.csv', row.names = F);
